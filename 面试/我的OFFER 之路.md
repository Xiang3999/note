...
>
> 什么协议使用UDP：NFS(网络文件系统)、SNMP(简单网络管理系统)、DNS(主域名称系统)、TFTP(通用文件传输协议)等
>
...
> > 这里说得不是太流畅，语言能力
>
> TCP三次握手，状态的转移（ACK-send,establish）
>
> 客户端一直在发送序列号不变的SYN包，server端会一直给客户端发送？
>
> > 我回答得是2 4 8s回ACK 
> >
> > 见：https://www.cnblogs.com/xiaolincoding/p/13067971.html
>
> C++ 编译过程
>
> > cc**命令如果不指定目标文件名时默认生成的可执行文件名为**a.out(linux)**或**a.exe(windows)**。可用 gcc [源文件名] -o [目标文件名] 来指定目标文件路径及文件名。
> >
> > 预编译期的作用：（1）删除注释； （2）处理预处理指令；（3）将包含的头文件展开。
> >
> > 编译期的作用：    （1）语法分析； （2）语义分析；           （3）汇总符号。
> >
> > 汇编期的作用：    （1）将汇编指令翻译成二进制文件；（2）生成符号表。
> >
> > 下面我们再来了解一下在Linux条件下编译期所使用的一些编译命令：
> >
> > 
> >
> > 编译期所做的工作是代码优化。汇编期所做的工作是将代码生成各种段。
> >
> > 链接期所做的工作是：
> >
> > （1）合并各个段；
...
> > （2）调整段的起始位置；
> >
> > （3）合并符号表，并进行符号分析（只会解析global，不会解析local）；
> >
> > （4）进行符号重定位工作。
>
> 为什么C++的模板都实现在头文件里面？
...
>
> 一个32位的数怎么快速的判断是否为2的幂次（[231. 2的幂](https://leetcode-cn.com/problems/power-of-two/)）
>
> > 1. 见32位转换为2进制，判断又几个1
> > 2. 一直除2
> > 3. 位运算     x & (-x)=x 来判断
...
> 数图遍历： BFS （队列） DFS（栈）
>
> TOP_K  堆，为什么用堆比较快，时间复杂度（nlogk）（这里脑抽说了n+logk）
>
> JAVA 底层的实现？ 垃圾回收？ 不了解
>
> 链表 ArrayList 的底层实现，说了hashmap
>
> sort()  能同时对8个apple排序，现在找出64最重的4个，最快的方法是？
>
> > 分8组，淘汰每组最后的4个  8次
> >
> > 在取8组里的top,进行排序，淘汰该序列最后4个的组  1次
> >
> > 最重的出来了
> >
> > 再选3个，从9个中选3个，2次
...
阿里云笔试，记忆深刻，两个hrad题一个都没有做

晚上难过得自暴自弃，很晚才睡觉

第二天还是难过，知道后面问了面试官，说这个只是一个参考，后面还是有面试的。


### 20210310

字节跳动北京的HR打来电话，说我的简历很适合他们岗位
...
> 链表转置  单向链表和双向链表
>
> 30位的十进制转位二进制，这里主要考察对Int的认识，long long int 是64位，又$2^{10}≈10^3$所以$10^{30}≈2^{100}>2^{64}$
>
> 所以这里要处理一下除法和取余
>
> 八股文
...
> 说了做了关于网络科学导论的？
>
> 说了一下去他们岗位可能要边学习Cpp然后边工作，Cpp基础不是很好。

### 20210315:triangular_flag_on_post:Aliyun:star:Bytedance

接到腾讯面试通知，明天下午进行HR面试，我很是疑惑，我还没有进行复试呀，怎么就这么快呢。该岗位是缺人吗？
...
> 块存储是收益占2/3的团队
>
> 主要问了关于case的一些复盘，自己是什么角色。发生了什么事情，为什么会发生，怎么处理的。
>
> 要真实，做事不要太认真或者太过于机械化。
...
### 20210316:triangular_flag_on_post:Tencent
...
https://join.qq.com/   已投简历

## Aliyun

https://talent.alibaba.com/  已投简历

## Bytedance

https://jobs.bytedance.com/  好像没有
